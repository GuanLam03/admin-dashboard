...
func (c *EditScheduleController) ShowSchedule(ctx http.Context) http.Response {
	...
	if err != nil {
		return ctx.Response().Json(400, http.Json{
			"error": models.ScheduleErrorMessage["invalid_request"],
		})
	}
	...
	if err := facades.Orm().Query().Where("id", id).First(&schedule); err != nil {
		return ctx.Response().Json(404, http.Json{
			"error": models.ScheduleErrorMessage["not_found"],
		})
	}
...
func (c *EditScheduleController) EditSchedule(ctx http.Context) http.Response {
	...
	if err != nil {
		return ctx.Response().Json(400, http.Json{
			"error":  models.ScheduleErrorMessage["invalid_start_at"],
		})
	}
	...
	if err != nil {
		return ctx.Response().Json(400, http.Json{
			"error": models.ScheduleErrorMessage["invalid_end_at"],
		})
	}
    ...
    if input.Status == "active" {
        if schedule.GoogleEventID == nil {
            ...
            if err != nil {
                return ctx.Response().Json(500, map[string]string{"error": models.ScheduleErrorMessage["google_insert_failed"] + ": " + err.Error()})
            }
            ...
            if err != nil {
                return ctx.Response().Json(500, map[string]string{"error": models.ScheduleErrorMessage["google_update_failed"] + ": " + err.Error()})
            }
        }
        ...
        if schedule.GoogleEventID != nil {
			...
			if err != nil {
                return ctx.Response().Json(500, map[string]string{"error": models.ScheduleErrorMessage["google_delete_failed"] + ": " + err.Error()})
            }
	...
	if err := facades.Orm().Query().Save(&schedule); err != nil {
		return ctx.Response().Json(500, http.Json{
			"error": models.ScheduleErrorMessage["update_failed"],
		})
	}
