...
func (c *TwoFAController) GenerateQRCode(ctx http.Context) http.Response {
    ...
    if err := facades.Auth(ctx).User(&user); err != nil {
        return ctx.Response().Json(401, http.Json{
            "error": models.UserErrorMessage["unauthorized"],
        })
    }

    // if already enabled don’t allow regenerate
    if user.TwoFactorEnabled {
        return ctx.Response().Json(400, http.Json{"error": models.TwofaErrorMessage["already_enabled"],})
    }
    ...
    if err != nil {
        return ctx.Response().Json(500, http.Json{
            "error": models.TwofaErrorMessage["generate_failed"],
        })
    }
    ...
    if err != nil {
        return ctx.Response().Json(500, http.Json{
            "error": models.TwofaErrorMessage["encrypt_failed"],
        })
    }
    ...
    if err := facades.Orm().Query().Save(&user); err != nil {
        return ctx.Response().Json(500, http.Json{
            "error":  models.TwofaErrorMessage["save_failed"],
        })
    }
    ...
    if err != nil {
        return ctx.Response().Json(500, http.Json{
            "error": models.TwofaErrorMessage["qr_failed"],
        })
    }
...
func (c *TwoFAController) ConfirmEnable(ctx http.Context) http.Response {
    ...
    if err != nil {
        return ctx.Response().Json(500, http.Json{
            "error":models.TwofaErrorMessage["decrypt_failed"],
        })
    }

    if !totp.Validate(code, secret) {
        return ctx.Response().Json(400, http.Json{
            "error": models.TwofaErrorMessage["invalid_code"],
        })
    }
...
func (c *TwoFAController) ConfirmDisable(ctx http.Context) http.Response {
    ...
    if !user.TwoFactorEnabled || user.TwoFactorSecret == "" {
        return ctx.Response().Json(400, http.Json{
            "error": models.TwofaErrorMessage["not_enabled"],
        })
    }
